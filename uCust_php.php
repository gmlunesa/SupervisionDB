...
if (!empty($_POST['Cust_FName'])) {
    ...
    if (!mysql_query("UPDATE customer SET Cust_FName = '$valueF' WHERE Cust_ID = '$valueCID' ")) {
...
if (!empty($_POST['Cust_LName'])) {
    ...
    // make a query. specify the $variables that we just made above.
    // please just change the SET statement
    if (!mysql_query("UPDATE customer SET Cust_LName = '$valueL' WHERE Cust_ID = '$valueCID' ")) {
        die('Error: ' . mysql_error());
    }
}
...
if (!empty($_POST['Cust_Num'])) {
    ...
    // make a query. specify the $variables that we just made above.
    // please just change the SET statement
    if (!mysql_query("UPDATE customer SET Cust_Num = '$valueNum' WHERE Cust_ID = '$valueCID' ")) {
        die('Error: ' . mysql_error());
    }
}
...
if (!empty($_POST['Cust_Email'])) {
    ...
    // make a query. specify the $variables that we just made above.
    // please just change the SET statement
    if (!mysql_query("UPDATE customer SET Cust_Email = '$valueEmail' WHERE Cust_ID = '$valueCID' ")) {
        die('Error: ' . mysql_error());
    }
}
